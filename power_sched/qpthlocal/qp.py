...
def QPFunction(eps=1e-12, verbose=0, notImprovedLim=3,
    ...
    class QPFunctionFn(Function):
        ...
        def backward(ctx, dl_dzhat):
            ...
            Q, p, G, h, A, b, zhats, dl_dzhat = Q.to(device), p.to(device), G.to(device), h.to(device), A.to(device), b.to(device), zhats.to(device), dl_dzhat.to(device)
            ...
            dx, _, dlam, dnu = pdipm_b.solve_kkt(
                ...
                dl_dzhat, torch.zeros(nBatch, nineq, device=device).type_as(G),
                torch.zeros(nBatch, nineq, device=device).type_as(G),
                torch.zeros(nBatch, neq, device=device).type_as(G) if neq > 0 else torch.Tensor())

            dps = dx
            dGs = bger(dlam, zhats) + bger(ctx.lams.to(device), dx)
            if G_e:
                dGs = dGs.mean(0)
            dhs = -dlam
            if h_e:
                dhs = dhs.mean(0)
            if neq > 0:
                dAs = bger(dnu, zhats) + bger(ctx.nus.to(device), dx)
                dbs = -dnu
                if A_e:
                    dAs = dAs.mean(0)
                if b_e:
