...
from cvxpy_powers_sched_kkt import cvxpy_ps_slack, cvxpy_ps_parallel_kkt
import wandb
...
class DiffusionCvxpyModule(torch.nn.Module):
    def __init__(self, diffusion, params, mc_samples, num_iter=100, distr_est=True, resample=False):
        ...
        self.cvxpy_layer = cvxpy_ps_parallel_kkt(params, self.mc_samples)
        # self.cvxpy_layer =  cvxpy_ps_slack(params, self.mc_samples)
        self.params = params
        self.n = params["n"]
        self.eps = 1e-12
        ...
        class DiffusionCvxpyFn(torch.autograd.Function):
            ...
            def backward(ctx, dl_dz_star):
                ...
                with torch.enable_grad():
                    _elbo_losses = self.diffusion.compute_elbo_grad_vec(_y_preds_sample, _x_sample, num_iter=self.num_iter, resample=resample)
