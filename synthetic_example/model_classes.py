...
class MLP_motivated(nn.Module):
    def __init__(self, y_dim, batch_size, device):
        super(MLP_motivated, self).__init__()
        # self.y_pred = nn.Parameter(
        #      torch.randn(batch_size, y_dim, device=device)
        # )
        self.y_pred = nn.Parameter(torch.randn(y_dim, device=device))
        self.batch_size = batch_size

    def forward(self, sample_size=None):
        return self.y_pred.unsqueeze(0).expand(sample_size, -1)
...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.bias_only = nn.Parameter(torch.zeros(output_dim))
        for p in self.net.parameters():
            p.requires_grad = False
            p.data.zero_()

    def forward(self, x):
        return self.bias_only.expand(x.size(0), -1)
