...
def run_rmse_net(model, pretrain_loader, pretrain_validate_loader, pretrain_optimizer, base_save, args, device, params):
    n = args.n
    x_dim = 28 * n
    alpha = params["alpha"]
    model = PortfolioPolicyNet(x_dim, n)
    for i in range(100):
        for batch_idx, (features, covariance_mat, labels) in enumerate(pretrain_loader):
            ...
            model.train()
            train_loss = _dfl_loss(labels, model(features), alpha)
            train_loss.backward()
            pretrain_optimizer.step()

        model.eval()
        test_loss = _dfl_loss(labels, model(features), alpha)
        if i % 200 == 0:
...
def train_portfolio_policy(model, optimizer, epoch, dataset, params, device='cpu', evaluate=False):
    model.train()
    alpha = params["alpha"]
    ...
    with tqdm.tqdm(dataset) as tqdm_loader:
        for batch_idx, (features, covariance_mat, labels) in enumerate(tqdm_loader):
            ...
            y_preds = model(features.float()) # (50, 28)
            dfl_loss = _dfl_loss(labels, y_preds.float(), alpha)
...
@torch.no_grad()
def validate_portfolio_policy(model, epoch, dataset, params, device='cpu', evaluate=False):
    model.eval()
    alpha = params["alpha"]
    ...
    with tqdm.tqdm(dataset) as tqdm_loader:
        for batch_idx, (features, covariance_mat, labels) in enumerate(tqdm_loader):
            ...
            y_preds = model(features.float()) # (50, 1)
            dfl_loss = _dfl_loss_linear(labels, y_preds.float())
 
            validate_objs.append(dfl_loss.item())
...
@torch.no_grad()
def test_portfolio_policy(model, epoch, dataset, params, device='cpu', evaluate=False):
    model.eval()
    alpha = params["alpha"]
    loss_fn = torch.nn.MSELoss()
    ...
    with tqdm.tqdm(dataset) as tqdm_loader:
        for batch_idx, (features, covariance_mat, labels) in enumerate(tqdm_loader):
            ...
            test_objs.append(dfl_loss.item())
